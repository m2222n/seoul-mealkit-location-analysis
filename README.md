...
| Deep Learning | 0.541 | 0.625 | 0.476 | 0.653 | - |

#### 회귀 모델 (1인당 평균 방문수 예측)

| 모델 | MSE | RMSE | MAE | R² | 비고 |
|------|-----|------|-----|-----|------|
| **Linear Regression** | **0.126** | 0.355 | 0.277 | -0.016 | MSE 최저 |
| SVR | 0.152 | 0.390 | 0.317 | -0.226 | 과적합 |
| KNN Regressor | 0.147 | 0.384 | 0.307 | -0.185 | |
| Random Forest | 0.165 | 0.407 | 0.341 | -0.330 | 과적합 |
| Deep Learning | 0.140 | 0.374 | 0.302 | -0.123 | |

> **채택 모델**: 분류 모델 (SVC F1=0.667)
> - 회귀 모델은 R² 전부 음수로 예측력 한계
> - 분류 확률값으로 순위화하여 입지 추천에 활용
...
| 피처 | 상위 15개 | 하위 15개 | 차이 | 해석 |
|------|----------|----------|------|------|
| 경제 | -0.655 | 0.593 | -1.249 | 상위 지역이 경제 지표 낮음 |
| 인프라 | -0.571 | 0.458 | -1.030 | 상위 지역이 인프라 적음 |
| 버스 이용량 | -0.564 | 0.412 | -0.977 | 상위 지역이 대중교통 적음 |
| 가구 | -0.159 | 0.640 | -0.799 | 상위 지역이 총 가구수 적음 |

**인사이트**:
- **상위 지역**: 대형 상업시설이 적은 주거 밀집 지역 → 밀키트 수요 높음
- **하위 지역**: 인프라/경제 발달한 상업 지역 → 외식 옵션 많아 밀키트 수요 낮음
- 역설적으로 "불편한 지역"이 밀키트 입지로 더 적합
...
├── data/
│   ├── raw/                      # 원본 공공데이터 (12개)
│   └── processed/                # 전처리된 데이터 (2개: train, test)
├── outputs/
│   ├── figures/                  # 시각화 결과물
│   └── models/                   # 학습된 모델 파일
├── docs/                         # 기획안 및 발표자료
...
5. 모델링
   ├── 분류: Logistic Regression, SVC, KNN, Random Forest, Deep Learning
   ├── 회귀: Linear Regression, SVR, KNN Regressor, RF Regressor, Deep Learning
   └── 채택: 분류 모델 (SVC F1=0.667, 회귀 R² 음수로 기각)
