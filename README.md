...
| **기법** | PCA, Linear Regression, SVR, KNN, Deep Learning |
...
| 모델 | MSE | Train Score | Test Score | 비고 |
|------|-----|-------------|------------|------|
| Linear Regression | 0.222 | 0.097 | -0.085 | 기각 |
| SVR | 0.225 | 0.862 | -1.356 | 과적합, 기각 |
| KNN Regressor | 0.232 | - | - | 기각 |
| **Deep Learning** | **0.142** | - | - | **최종 채택** |

> Deep Learning 모델 Loss: 0.1417, 최대 오차: 0.835
...
| 교통량 | 평균 버스 이용량 |
| 수요량 | 사업체 수, 외국인 수 |
| 인프라 | 인근 음식점/편의점/대형마트 수 |
| 경제 | 23년 공시지가, 배달 금액 총수 |
| 가구 | 1~2인 가구, 3인 이상 가구, 주택 수 |

### 상위 vs 하위 지역 특성 비교

![District Comparison](outputs/figures/district_comparison.png)

- **상위 15개 지역**: 30~59세 인구, 3인가구 이상, 비만도 분포 높음
- **하위 15개 지역**: 1~2인가구, 인프라, 경제 지표 높음 (역설적)
...
**상위 지역 (1인당 평균 방문수 기준):**
- 난곡동, 사당1동, 수유3동, 신림동, 서원동, 인헌동, 신원동, 상도3동, 미성동, 낙성대동...

**최종 선정 지역:**
- **인헌동, 보라매동** (인프라 + 버스 이용량 최적 조합)
...
5. 모델링
   ├── Linear Regression, SVR, KNN (기각)
   └── Deep Learning (채택, Loss: 0.1417)
